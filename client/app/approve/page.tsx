...
const DeviceApprovalPage = () => {
  ...
  const handleDeny = async () => {
    // Preemptive safety check: Bails early if no query variable is found
    if (!userCode) {
        toast.error("Invalid authorization context: Missing device tracking code.");
        return;
    }
          ...
          <div className="border-2 border-dashed border-zinc-700 rounded-2xl p-8 bg-zinc-900/50 backdrop-blur-sm text-center">
            <div className="flex justify-center mb-6">
              <div className="relative ">
                <div className=" w-24 h-24 rounded-2xl border-2 border-dashed border-zincd-600 bg-zinc-800 flex items-center justify-center">
                  <Smartphone className="w-12 h-12 text-cyan-400" />
                </div>
                <div className="absolute flex items-center justify-center  -top-2 -right-2 w-8 h-8 bg-orange-500 rounded-full border-2 border-zinc-900">
            ...
            <div className="space-y-3 ">
              ...
              <p className="text-sm text-zinc-400">
                A new device is requesting access to you account
              </p>
            </div>
          </div>
